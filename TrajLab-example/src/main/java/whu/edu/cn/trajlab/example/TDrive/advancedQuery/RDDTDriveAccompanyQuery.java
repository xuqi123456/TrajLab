 ...
 * @date 2024/04/13
 */
public class RDDTDriveAccompanyQuery {
    public static void main(String[] args) throws IOException {
        ...
        conf.set(TIME_THRESHOLD, tdis);
        conf.set(TIME_PERIOD, tunit);
        conf.set(K, k);
        AdvancedQuery advancedQuery = new AdvancedQuery(conf);
        long startAll = System.currentTimeMillis();
        try (SparkSession sparkSession =
                     SparkSessionUtils.createSession(RDDTDriveAccompanyQuery.class.getName(), true, par)) {
