...
import static whu.edu.cn.trajlab.base.trajectory.Trajectory.Schema.*;
...
public class TrajectoryDataMapper {
    ...
    public static List<Tuple2<KeyFamilyQualifier, KeyValue>> mapPutToKeyValue(Put put)
        ...
        String[] dataFrame;
        if (put.has(DBConstants.COLUMN_FAMILY, DBConstants.PTR_QUALIFIER)) {
        ...
        for (String frame : dataFrame) {
            if(put.has(DBConstants.COLUMN_FAMILY, Bytes.toBytes(frame))){
                List<Cell> cells = put.get(DBConstants.COLUMN_FAMILY, Bytes.toBytes(frame));
                Result result = Result.create(cells);
                byte[] quaFilterValue = result.getValue(DBConstants.COLUMN_FAMILY, Bytes.toBytes(frame));
                KeyValue keyValue = new KeyValue(put.getRow(), DBConstants.COLUMN_FAMILY, Bytes.toBytes(frame),
                        quaFilterValue);
                byte[] row = result.getRow();
                KeyFamilyQualifier keyFamilyQualifier = new KeyFamilyQualifier(result.getRow(), DBConstants.COLUMN_FAMILY,
                        Bytes.toBytes(frame));
