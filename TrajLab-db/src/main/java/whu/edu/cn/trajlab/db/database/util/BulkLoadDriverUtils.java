...
import whu.edu.cn.trajlab.db.database.driver.TableBulkLoadDriver;
import whu.edu.cn.trajlab.db.database.driver.TextBulkLoadDriver;
import whu.edu.cn.trajlab.db.database.mapper.MainToMainMapper;
import whu.edu.cn.trajlab.db.database.mapper.MainToSecondaryMapper;
import whu.edu.cn.trajlab.db.database.mapper.TextToMainMapper;
...
public class BulkLoadDriverUtils {
    ...
    public static void createIndexFromTable(Configuration conf, IndexMeta indexMeta, DataSetMeta dataSetMeta) throws IOException {
        ...
        job.getConfiguration().set(DBConstants.BULKLOAD_TARGET_INDEX_NAME, outTableName);
        job.getConfiguration().setBoolean(DBConstants.ENABLE_SIMPLE_SECONDARY_INDEX, indexMeta.getIndexTableName().contains("simple"));
    ...
    public static void createIndexFromFile(Configuration conf, IndexTable indexTable, Class parser) throws IOException {
        Path inPath = new Path(conf.get(DBConstants.BULK_LOAD_INPUT_FILE_PATH_KEY));
        Path outPath = new Path(conf.get(DBConstants.BULK_LOAD_TEMP_FILE_PATH_KEY));
        ...
        job.getConfiguration().setClass(DBConstants.BULKLOAD_TEXT_PARSER_CLASS, parser, TextTrajParser.class);
        job.getConfiguration().set(DBConstants.BULKLOAD_TARGET_INDEX_NAME, tableName);
    ...
    public static IndexTable getIndexTable(Configuration conf) throws IOException {
        String tableName = conf.get(DBConstants.BULKLOAD_TARGET_INDEX_NAME);
    ...
    public static TextTrajParser getTextParser(Configuration conf) throws InstantiationException, IllegalAccessException {
        Class cls = conf.getClass(DBConstants.BULKLOAD_TEXT_PARSER_CLASS, null);
