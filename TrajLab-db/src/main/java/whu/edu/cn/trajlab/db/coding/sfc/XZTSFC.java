...
public class XZTSFC implements Serializable {
  ...
  public long index(TimeLine timeLine, TimeBin timeBin) {
    ...
    int l1 = (int) Math.floor(Math.log(length) / CodingConstants.LOG_FIVE);
    int level;
    if (l1 >= g) {
      level = g;
    } else {
    ...
    for (TimeBin timeBin : timeBinList) {
      ...
      while (level < g && !remaining.isEmpty()) {
        ...
        if (next.equals(levelSeparator)) {
          ...
          if (isContainQuery) {
            ranges =
                containSequenceCodeRange(next, referencedQuery, level, ranges, remaining, timeBin);
          } else {
            ranges =
                overlapSequenceCodeRange(next, referencedQuery, level, ranges, remaining, timeBin);
          }
        }
      }
